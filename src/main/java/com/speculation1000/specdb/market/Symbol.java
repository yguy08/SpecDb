...
public class Symbol implements Comparable<Symbol> {
	...
	public Symbol(String base,String exchange){
		this.base = base;		
		if(!base.equalsIgnoreCase("BTC")){
			this.counter = "BTC";
			this.exchange = exchange;
		}else{
			this.counter = "USDT";
		...
		this.symbol = this.base+"/"+this.counter+":"+this.exchange;
	}
	...
	public Symbol(String symbol){
		if(symbol!=null && !symbol.isEmpty()){
			...
			if(base != -1 && counter != -1){
				...
				this.exchange = symbol.substring(counter+1);
				this.symbol = symbol;
			}else{
				this.base = UNKNOWN;
